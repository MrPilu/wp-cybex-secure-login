...
if ( ! class_exists( 'Cbxlogin_Denylist' ) ) {
	class Cbxlogin_Denylist extends WP_List_Table {
		...
		function column_ip( $item ) {
			...
			$actions = array(
				'delete'	=> '<a href="' . wp_nonce_url( sprintf( '?page=%s&cbxsec_remove_from_denylist=%s', $_REQUEST['page'], $item['ip'] ), 'cbxsec_remove_from_denylist_' . $item['ip'], 'cbxsec_nonce_name' ) . '">' . __( 'Delete', 'cybex-security' ) . '</a>'
			);
		...
		function get_bulk_actions() {
			/* adding bulk action */
			$actions = array(
				'remove_from_denylist_ips'	=> __( 'Delete', 'cybex-security' )
			);
			return $actions;
		}
		...
		function action_message() {
			...
			if ( isset( $_REQUEST['cbxsec_remove_from_denylist'] ) ) {
				check_admin_referer( 'cbxsec_remove_from_denylist_' . $_REQUEST['cbxsec_remove_from_denylist'], 'cbxsec_nonce_name' );
				$ip_list = $_REQUEST['cbxsec_remove_from_denylist'];
			} else {
				if(
			...
			if ( isset( $ip_list ) ) {
				if ( empty( $ip_list ) ) {
					...
					if ( $wpdb->last_error ) {
						$action_message['error'] = $ips . '&nbsp;-&nbsp;' . __( 'Error while deleting from deny list', 'cybex-security' );
					} else {
						$done_ips = (array)$ip_list;
						$action_message['done'] = implode( ', ', $done_ips ) . '&nbsp;' . ( 1 == count( $done_ips ) ? __( 'has been deleted from deny list', 'cybex-security' ) : __( 'have been deleted from deny list', 'cybex-security' ) );
						if ( 1 == $cbxsec_options["block_by_htaccess"] ) {
							do_action( 'cbxsec_htaccess_hook_for_reset_block', $done_ips );
						}
					}
				}
			}
